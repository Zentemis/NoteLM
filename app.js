...
const CONFIG = {
  ...
  sampleRate: 24000,
  lineGap: 0.35, // seconds of silence between lines
};
...
const dom = {
  ...
  // Lazy lookups for elements added after parse
...
// ===== Helpers =====
const generateId = prefix => prefix + Date.now() + Math.random().toString(36).slice(2, 5);
...
const voiceLabel = vid => { const v = VOICES[vid]; return v ? `${v.name} (${v.gender[0]})` : vid; };
...
async function generate() {
  ...
  try {
    ...
    for (let i = 0; i < valid.length; i++) {
      ...
      const audio = await kokoroModel.generate(line.text, { voice: spk?.voice || 'af_heart' });
...
function createBufferSource() {
  ...
  source.onended = () => { isPlaying = false; dom.playPauseBtn.textContent = '▶'; cancelAnimationFrame(animFrameId); };
  return source;
}
...
function playAudio() {
  ...
  currentSource = createBufferSource();
...
// Paste modal — use the named functions
